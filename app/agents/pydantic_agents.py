...
class PydanticTradingAgentSystem:
    ...
    def _create_agents(self):
        ...
        # Trading Signal Agent — enforces TradingSignal and RiskLevel enum usage
        signal_agent = Agent(
            model=self.model_name,
            deps_type=Dependencies,
            output_type=TradingDecision
        ...
        @signal_agent.system_prompt
        def signal_system_prompt(ctx: RunContext[Dependencies]) -> str:
            return f"""You are a Trading Signal Agent that generates precise, type-safe trading signals for {ctx.deps.symbol}.

            Your role:
            - Produce a single, definitive trading signal using the TradingSignal enum: BUY, SELL, or HOLD
            - Classify risk using the RiskLevel enum: LOW, MEDIUM, or HIGH
            - Back every signal with Fibonacci levels, technical indicators, and sentiment data
            - Provide specific entry and exit price targets

            IMPORTANT: The decision field MUST be one of: BUY, SELL, HOLD.
            The risk_level field MUST be one of: LOW, MEDIUM, HIGH."""

        @signal_agent.tool
        def get_market_data(ctx: RunContext[Dependencies], period: str = "1mo") -> str:
            """Fetch stock data with technical indicators for signal generation"""
            stock_data = get_stock_data(ctx.deps.symbol, period, data=ctx.deps.data)
            return f"Stock data retrieved: {stock_data.model_dump_json()}"

        @signal_agent.tool
        def get_fibonacci_analysis(ctx: RunContext[Dependencies], lookback_days: int = 20) -> str:
            """Calculate Fibonacci retracement levels for key entry/exit targets"""
            fib_data = calculate_fibonacci_levels(ctx.deps.symbol, lookback_days)
            return f"Fibonacci analysis: {fib_data.model_dump_json()}"

        @signal_agent.tool
        def get_sentiment(ctx: RunContext[Dependencies], timeframe: str = "3d") -> str:
            """Analyze market sentiment to confirm signal direction"""
            sentiment_data = analyze_market_sentiment(ctx.deps.symbol, timeframe)
            return f"Sentiment analysis: {sentiment_data.model_dump_json()}"

        @signal_agent.tool
        def save_signal_audit(ctx: RunContext[Dependencies], action: str, confidence: float, rationale: str, risk_level: str = "MEDIUM") -> str:
            """Save trading signal decision to audit trail"""
            return save_audit_entry(ctx.deps.symbol, "SIGNAL", action, confidence, rationale, risk_level=risk_level)
    ...
    def run_signal_analysis(self, symbol: str, data: pd.DataFrame) -> Dict[str, Any]:
        """Run Trading Signal Agent — demonstrates type-safe TradingSignal and RiskLevel enums"""
        try:
            deps = Dependencies(symbol=symbol, data=data)

            signal_prompt = f"""
            Generate a precise, type-safe trading signal for {symbol}:
            1. Fetch stock data and technical indicators using get_market_data tool
            2. Calculate Fibonacci retracement levels using get_fibonacci_analysis tool
            3. Confirm signal direction with get_sentiment tool
            4. Choose decision from TradingSignal enum: BUY, SELL, or HOLD
            5. Choose risk_level from RiskLevel enum: LOW, MEDIUM, or HIGH
            6. Set specific entry_price and exit_price targets based on Fibonacci levels
            7. Save your signal using save_signal_audit tool

            Return a TradingDecision where decision is a TradingSignal enum value
            and risk_level is a RiskLevel enum value.
            """

            signal_result = self.agents["signal_agent"].run_sync(signal_prompt, deps=deps)
            output = signal_result.output

            return {
                "agent": "signal_agent",
                "analysis": output,
                "signal": output.decision.value if hasattr(output.decision, 'value') else str(output.decision),
                "risk_level": output.risk_level.value if hasattr(output.risk_level, 'value') else str(output.risk_level),
                "confidence": output.confidence if hasattr(output, 'confidence') else 0.75
            }
