...
def get_pydantic_agents():
    return PydanticTradingAgentSystem(use_openai=Config.USE_OPENAI)
...
def run_regulatory_agent(symbol):
    ...
    try:
        ...
        if hasattr(compliance_data, 'explanation'):
            ...
            violations = list(compliance_data.violations) if hasattr(compliance_data, 'violations') and compliance_data.violations else []
        else:
...
def run_signal_agent():
    """Run Trading Signal Agent — surfaces all TradingDecision enum fields without string parsing"""
    try:
        pydantic_system = get_pydantic_agents()
        print(f"📊 Running Trading Signal Agent for {st.session_state.symbol}...")

        signal_results = pydantic_system.run_signal_analysis(
            st.session_state.symbol, st.session_state.data
        )

        if "error" in signal_results:
            return {"error": signal_results["error"]}

        # Read directly from the validated TradingDecision Pydantic model — no string parsing
        decision = signal_results["analysis"]

        return {
            "signal": signal_results["signal"],         # TradingSignal enum → "BUY"/"SELL"/"HOLD"
            "risk_level": signal_results["risk_level"], # RiskLevel enum → "LOW"/"MEDIUM"/"HIGH"
...
if 'signal_analysis' not in st.session_state:
    st.session_state.signal_analysis = None
...
signal_button = st.sidebar.button("📊 Trading Signal Agent", use_container_width=True, key="signal_btn")
...
if 'data' in st.session_state and st.session_state.data is not None and not st.session_state.data.empty:
    ...
    if signal_button:
        with st.spinner("📊 Running Trading Signal Agent..."):
            ...
            if "error" in result:
                ...
                st.session_state.signal_analysis = result
    ...
    if market_button or strategy_button or risk_button or regulatory_button or supervisor_button or signal_button or trade_button:
...
with col2:
    ...
    # Trading Signal Agent Results — validated TradingDecision object
    if 'signal_analysis' in st.session_state and st.session_state.signal_analysis:
        with st.expander("📊 Trading Signal Agent Results — TradingDecision", expanded=False):
            result = st.session_state.signal_analysis
            # TradingSignal and RiskLevel enum values rendered with colour
            sig_col, risk_col = st.columns(2)
            with sig_col:
                sig = result['signal']
                if sig == "BUY":
                    st.success(f"Signal (TradingSignal): {sig}")
                elif sig == "SELL":
                    st.error(f"Signal (TradingSignal): {sig}")
                else:
                    st.info(f"Signal (TradingSignal): {sig}")
            with risk_col:
                risk = result['risk_level']
                if risk == "HIGH":
                    st.error(f"Risk (RiskLevel): {risk}")
                elif risk == "MEDIUM":
                    st.warning(f"Risk (RiskLevel): {risk}")
                else:
                    st.success(f"Risk (RiskLevel): {risk}")
            st.write(f"**Confidence:** {result['confidence']:.1%}")
            ...
            if result.get('position_size'):
                st.write(f"**Position Size:** {result['position_size']:.1f}%")
            st.write("**Rationale:**")
